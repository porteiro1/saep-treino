...
if($connect -> connect_errno){
  ...
  $dataEntrega = $connect -> real_escape_string($_POST['dataEntrada']);

  $sql = "SELECT * FROM products WHERE id = " . $id;
  $results = $connect -> query($sql);
  $row = $results -> fetch_array();

  if(($row['quantity'] - $quantity) <=3){
    echo "estoque baixo";
    ...
    $sqlLending = "INSERT INTO lending (dataEmprestimo,idProduct) VALUES ('$dataEntrega', $id)";
    $connect ->query($sqlLending);

    $idEmprestimo = $connect->insert_id;
